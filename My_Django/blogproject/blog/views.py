...
class PostDetailView(DetailView):
    """数据库获取模型的一条记录数据也是常见的需求。比如查看某篇文章的详情，就是从数据库中获取这篇文章的记录然后渲染模板。
...
# 展示全部文章
class PostAllView(SetHeadlineMixin,ListView):
    headline = '归档'
    model = Post
...
def category(request,pk):
    # 记得导入Category ,et_object_or_404 函数和 detail 视图中一样，其作用是如果用户访问的分类不存在，
...
def About(request):
    dict = {"name" : "风之小云天","wechat" : "wu30135681" ,
            "email" : "1397991131@q.com"}
...
def Contact(request):
    ...
    try:
        if request.method=='POST':
            ...
            contact = models.Contact.objects.create(name=post_name,email=post_email,message=post_message)
            contact.save()
    except:
        alert = '网站出现异常，请稍后重试'
